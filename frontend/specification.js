/**
 * Specification Creation System - Frontend Application
 * 
 * Features:
 ...
 * - Uses dedicated Specification Creation Agent
 */
...
class KnowledgeQueryApp {
    ...
    initializeAuthManager() {
        // window.specificationAuthManagerが利用可能になるまで待機
        const checkAuthManager = () => {
            if (window.specificationAuthManager) {
                this.authManager = window.specificationAuthManager;
                console.log('[SpecificationApp] Auth manager initialized');
            } else {
    ...
    initializeChatSession() {
        ...
        // Check if chat session ID exists in sessionStorage（仕様書モード専用）
        let sessionId = sessionStorage.getItem('specificationSessionId');
        if (!sessionId) {
            sessionId = 'specification_' + generateUUID();
            sessionStorage.setItem('specificationSessionId', sessionId);
            console.log('[initializeChatSession] Generated new specification session_id:', sessionId);
        } else {
            console.log('[initializeChatSession] Using existing specification session_id:', sessionId);
        }
    ...
    loadPanelSizeFromStorage() {
        try {
            const savedChatWidth = localStorage.getItem('specification-chat-width');
            const savedPdfWidth = localStorage.getItem('specification-pdf-width');
    ...
    savePanelSizeToStorage() {
        try {
            ...
            localStorage.setItem('specification-chat-width', chatWidth);
            localStorage.setItem('specification-pdf-width', pdfWidth);
