/**
 * Knowledge Query System - Frontend Application
 * 
 * Features:
...
class KnowledgeQueryApp {
    constructor(config = {}) {
        ...
        this.useAgent = true;  // Agent利用フラグ（デフォルト: true）
        this.templateAgentType = 'QUERY_SUPPORT';  // テンプレートAPI用
    ...
    initializeAuthManager() {
        // window.knowledgeQueryAuthManagerが利用可能になるまで待機
        const checkAuthManager = () => {
            if (window.knowledgeQueryAuthManager) {
                this.authManager = window.knowledgeQueryAuthManager;
                console.log('[KnowledgeQueryApp] Auth manager initialized');
            } else {
    ...
    initializeChatSession() {
        ...
        // Check if chat session ID exists in sessionStorage
        let sessionId = sessionStorage.getItem('chatSessionId');
        if (!sessionId) {
            sessionId = generateUUID();
            sessionStorage.setItem('chatSessionId', sessionId);
            console.log('[initializeChatSession] Generated new chat_session_id:', sessionId);
        } else {
            console.log('[initializeChatSession] Using existing chat_session_id:', sessionId);
        }
     ...
     */
    loadSettings() {
        const savedUseAgent = localStorage.getItem('useAgent');
        if (savedUseAgent !== null) {
            this.useAgent = savedUseAgent === 'true';
        } else {
            // デフォルト: Agentを利用する
            this.useAgent = true;
        }
        console.log('[loadSettings] useAgent:', this.useAgent);
    }
    ...
    initializeElements() {
        this.elements = {
            ...
            useAgentToggle: document.getElementById('useAgentToggle'),
    ...
    loadPanelSizeFromStorage() {
        try {
            const savedChatWidth = localStorage.getItem('knowledge-query-chat-width');
            const savedPdfWidth = localStorage.getItem('knowledge-query-pdf-width');
    ...
    savePanelSizeToStorage() {
        try {
            ...
            localStorage.setItem('knowledge-query-chat-width', chatWidth);
            localStorage.setItem('knowledge-query-pdf-width', pdfWidth);
    ...
    async submitQuery() {
        ...
        try {
            ...
            console.log('[submitQuery] Using Agent:', this.useAgent);
